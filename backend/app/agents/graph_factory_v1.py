...
from app.agents.state import BackendState, DebateState
...
from app.lib.graph.subgraphs.debate_subgraph import debate_subgraph
...
async def debate_bridge_node(state: BackendState) -> Dict[str, Any]:
    """
    Ponte de integração entre o Grafo Principal e o Subgrafo de Debate.
    Prepara o estado do debate e executa o subgrafo.
    """
    log.info("--- ENTRANDO NA PONTE DE DEBATE ---")
    
    # Grounding para o debate
    canvas_summary = serialize_canvas_for_prompt(state.get("canvas_nodes", []))
    
    # Prepara o input para o subgrafo
    debate_input = DebateState(
        topic=state.get("debate_topic") or "Desenvolvimento do Projeto Académico",
        canvas_summary=canvas_summary,
        rag_context=None, # Implementar RAG se disponível futuramente
        turns=[],
        current_role="primaria",
        turn_order=["primaria", "complementar", "antagonista", "metodologica"],
    ...
    # Executa o subgrafo
    # NOTA: O streaming será capturado via events do langgraph_graph pai
    result = await debate_subgraph.ainvoke(debate_input)
    
    # Converte o resultado para o histórico do chat principal
    synthesis = result.get("synthesis", "Debate concluído sem síntese clara.")
    return {
        "messages": [AIMessage(content=f"## Síntese do Debate das Almas\n\n{synthesis}")],
        "debate_history": result.get("turns", []),
        "is_debate_mode": False # Reseta após conclusão
    }
...
def create_backend_graph():
    ...
    workflow.add_node("debate_bridge", debate_bridge_node)
    ...
    workflow.add_edge("debate_bridge", END)
