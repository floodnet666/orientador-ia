...
async def run_audit():
    ...
    try:
        ...
        version_info = await client.get_client_grpc().get_api_client().get_server_info()
        log.info(f"Versão reportada pelo Servidor: {version_info}")
        ...
        # 4. TESTE DE BUSCA LEGACY (SEARCH API)
        log.info("--- Fase 4: Busca Legacy (Search API) ---")
        legacy_search = await client.search(
            collection_name=AUDIT_COLLECTION,
            query_vector=("dense", [0.1, 0.2, 0.3, 0.4]),
            limit=1
        )
        if legacy_search and legacy_search[0].id == 1:
            log.info("Busca Legacy: SUCCESS")
        else:
            log.error(f"Busca Legacy: FAILED (Result: {legacy_search})")

        # 5. TESTE DE BUSCA UNIVERSAL (UQA / query_points) - GARGALO PROVÁVEL
        log.info("--- Fase 5: Universal Query API (query_points) ---")
        try:
        ...
        try:
            filter_search = await client.search(
                collection_name=AUDIT_COLLECTION,
                query_vector=("dense", [0.1, 0.2, 0.3, 0.4]),
            ...
            log.info("Nested Filters: SUCCESS")
        except Exception as e:
            log.error(f"Nested Filters: FAILED: {e}")
