...
# Sistema de prompts dedicados para Almas técnicas
...
async def run_audit():
    ...
    report_path = "/app/debate_audit_report_v4.md"
    start_time = time.perf_counter()

    with open(report_path, "w", encoding="utf-8") as f:
        f.write(f"# 🛡️ Relatório de Auditoria: RAG + PINNs (Teste 04)\n\n")
        f.write(f"**Data**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"**Provocação**: {user_provocation}\n\n")
        f.flush()

        logger.info("🚀 Iniciando Auditoria Teste 04...")
        current_turn = None
        turn_start = 0
